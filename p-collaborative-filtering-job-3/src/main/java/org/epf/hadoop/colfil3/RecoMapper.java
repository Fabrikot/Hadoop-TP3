...
public class RecoMapper extends Mapper<LongWritable, Text, Text, UserRecommendation> {
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        // Séparer les users et le nombre de relations communes
        String[] users = parts[0].split(",");
        if (users.length != 2) return;
        int commonFriends = Integer.parseInt(parts[1]);

        // Émettre les recommandations pour les deux utilisateurs
